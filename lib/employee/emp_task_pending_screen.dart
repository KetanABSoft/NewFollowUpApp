...
class EmpTaskPending extends StatefulWidget {
  const EmpTaskPending({Key? key}) : super(key: key);

  @override
  _EmpTaskPendingState createState() => _EmpTaskPendingState();
}

class _EmpTaskPendingState extends State<EmpTaskPending> {
                 ...
                 if(success)
                   {
                     ...
                     Navigator.push(context, MaterialPageRoute(builder: (context) => EmpTaskCompletedScreen(),));
                   }
                 else
                   {
  ...
  Future<List<Task>> pendingTask() async {
    final url = Uri.parse("http://103.159.85.246:4000/api/task/tasks/pendingByEmp");
    try {
