...
import 'package:followup/employee/emp_dashboard_screen.dart';
...
import 'admin_send_task.dart';
import 'admin_timesheeth.dart';
import 'admin_todays_task.dart';
...
// ];
  ...
  void _handleAddLead(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => AdminAddLead(id: '0', task: '')),
    );
  }
...
// );
  ...
  void _handleCard3Tap() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AdminTodaysTask(),
      ),
    );
  }
  ...
  void _handleCard4Tap() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AdminSendTaskScreen(),
      ),
    );
  }
  ...
  String officeIpAddress = '103.150.139.180';

  String? ipdata; // Declare ipdata as nullable string

  Future<void> fetchIpAddress() async {
    try {
      final ipResponse = await http.get(Uri.parse('https://api.ipify.org/?format=json'));
      if (ipResponse.statusCode == 200) {
        Map<String, dynamic> responseData = jsonDecode(ipResponse.body);
        ipdata = responseData['ip'];
        print('Fetched IP Address: $ipdata');
      } else {
        print('Failed to get IP address: ${ipResponse.statusCode}');
// Optionally, you can throw an error here or handle as needed.
      }
    } catch (e) {
      print('Exception getting IP address: $e');
// Handle specific types of errors if needed, e.g., SocketException, TimeoutException, etc.
    }
  }
  ...
  Future<String> postClockIn() async {
    await fetchIpAddress(); // Fetch IP address before proceeding

    if (ipdata == '103.150.139.180') { // Check if fetched IP matches office IP
      SharedPreferences preferences = await SharedPreferences.getInstance();
      ...
      try {
        ...
        } else if (response.statusCode == 403 && response.body.contains('Invalid token')) {
          print('Clock In Error: Invalid token');
          return 'Failed to clock in: Invalid token'; // Handle invalid token error
        } else {
          print('Clock In Error: ${response.statusCode}');
          print('Clock In Response Body: ${response.body}');
          return 'Failed to clock in: ${response.body}';
        }
      } catch (e) {
        print('Exception during clock in: $e');
        return 'Failed to clock in: $e';
      }
    } else {
  ...
  Future<String> postClockOut() async {
    ...
    if (ipdata == '152.57.214.185') { // Check if fetched IP matches office IP
      SharedPreferences preferences = await SharedPreferences.getInstance();
      String? email = preferences.getString('email');
      String? role = preferences.getString('role');
...
// Replace 'YOUR_AUTH_TOKEN' with your actual authentication token
        ...
        if (response.statusCode == 200) {
          ...
          return 'Failed to clock out: ${response.body}';
        }
      } catch (e) {
        print('Exception during clock out: $e');
        return 'Failed to clock out: $e';
      }
    } else {
      return 'Unauthorized IP';
    }
